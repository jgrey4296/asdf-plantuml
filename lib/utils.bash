...
GH_REPO="https://github.com/plantuml/plantuml"
TOOL_NAME="plantuml"
TOOL_TEST="plantuml -version"
...
function download_release() {
	...
	echo "* To: $ASDF_DOWNLOAD_PATH/$filename"
	curl "${curl_opts[@]}" -o "$ASDF_DOWNLOAD_PATH/$filename" -C - "$url" || fail "Could not download $url"

	test -f "$ASDF_DOWNLOAD_PATH/$filename" || fail "download file did not exist"
}
...
function install_version() {
	...
	(
		...
		java -jar "$install_path/$TOOL_NAME-$ASDF_INSTALL_VERSION.jar" -version || fail "Expected $install_path/$tool_cmd to be executable."
